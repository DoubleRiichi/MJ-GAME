...
function findTriplets(hand_holder: number[]): number[][] {
...
function findSequences(hand_holder: number[]) : number[][] {
    ...
    for(let i = 0; i < hand_holder.length; i++) {
        ...
        if(i < hand_holder.length - 2) {
            ...
            if(buffs.length == 3) {
                ...
                melds.push(buffs)
                i_remove.forEach( (key) => 
                    hand_holder.splice(key, 1))
...
function determineMelds(hand: number[]) : number[][] {
